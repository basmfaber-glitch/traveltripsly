document.addEventListener("DOMContentLoaded", () => {
  ...
  function resizeCanvas(){
    ...
    height = canvas.height = canvas.querySelector(".hero").clientHeight;
  }
  window.addEventListener("resize", () => { resizeCanvas(); initParticles(true); });
  resizeCanvas();
  ...
  canvas.addEventListener("mousemove",(e)=>{const rect=canvas.getBoundingClientRect();mouse.x=e.clientX-rect.left;mouse.y=e.clientY-rect.top;mouse.active=true;});
  ...
  function frame(){
    ...
    particles.forEach((p,i)=>{
      ...
      ctx.beginPath(); ctx.fillStyle = `rgba(${p.hue.r},${p.hue.g},${p.hue.b},${Math.min(1,alpha*1.6)})`; ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fill();
    });
    ...
    ctx.beginPath(); ctx.fillStyle = state.day?"rgba(3,70,120,0.9)":"rgba(0,160,255,0.98)";
    ctx.arc(orbX,orbY,8,0,Math.PI*2); ctx.fill();
  ...
  (function animateWaves(){
    ...
    function makePath(offset){
      const w=1440,h=120; let d=`M0 ${h+60}`; const segments=12;
      for(let i=0;i<=segments;i++){const x=(i/segments)*w; const y=Math.sin((i/segments)*Math.PI*2+offset)*18+h/2; d+=` L ${x} ${y}`;}
      d+=` L ${w} ${h+60} Z`; return d;
    }
    let off=0;
    function step(){off+=0.02; if(path) path.setAttribute("d",makePath(off));
      if(glowGroup){glowGroup.innerHTML=""; for(let i=0;i<3;i++){const p=document.createElementNS("http://www.w3.org/2000/svg","path"); p.setAttribute("d",makePath(off+i*0.6)); p.setAttribute("fill","none"); p.setAttribute("stroke","#00d1ff"); p.setAttribute("stroke-opacity","0.06"); p.setAttribute("stroke-width",String(80-i*20)); glowGroup.appendChild(p);}}
  ...
  // ---------- Fit canvas ----------
  function fit(){ const hero=document.querySelector(".hero"); if(!hero)return; const rect=hero.getBoundingClientRect(); canvas.width=Math.max(window.innerWidth,300); canvas.height=Math.max(rect.height,200); canvas.style.width="100%"; canvas.style.height=rect.height+"px"; }
  window.addEventListener("load",fit);
  window.addEventListener("resize",fit);
  fit(); setTimeout(()=>initParticles(true),250);
});
